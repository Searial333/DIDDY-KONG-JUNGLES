...
export function spawnPeanutProjectile(w: World, owner: EntityId) {
    ...
    // Position offset to align with gun painter
    const gunOffsetX = 48; // Adjusted from analysis of diddyPainter
    const spawnX = t.facing > 0 
        ? t.pos.x + gunOffsetX 
        : t.pos.x + t.size.x - gunOffsetX - size.x;
        
    const spawnY = t.pos.y + t.size.y / 2 + 4;

    set<Transform>(w, 'transform', e, {
        pos: { x: spawnX, y: spawnY },
        vel: { x: t.facing * 900, y: 0 },
        size,
        facing: t.facing,
        onGround: false,
        onWall: 0,
    ...
    // Visual FX
    w.actions.createParticleBurst(spawnX + (t.facing > 0 ? size.x : 0), spawnY + size.y/2, 5, '#FFF', 'burst');
