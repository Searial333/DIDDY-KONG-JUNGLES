...
export function abilitySystem(w: World, input: InputState) {
    ...
    const lockedStates = ['slamming', 'rolling', 'rollWindup', 'backflip', 'winded', 'dashing', 'slamLand', 'wallSliding', 'jetpack'];
    const canAct = !lockedStates.includes(s.state) || isAttacking;

    // --- SHOOTING ---
    if (input.shootDown && a.available.has('bottleBlaster') && !['rolling', 'dashing', 'slamming', 'wallSliding', 'climbing', 'in_barrel'].includes(s.state)) {
    ...
    // --- JETPACK (Hover) ---
    if (input.hover && !t.onGround && a.available.has('jetpack') && s.state !== 'rolling' && s.state !== 'dashing' && s.state !== 'wallSliding') {
    ...
    // --- CLIMBING ---
    if (t.onLadder && !isAttacking && s.state !== 'rolling' && s.state !== 'dashing' && s.state !== 'jetpack') {
    ...
    if (input.dashDown && canDash && !['dashing', 'slamming', 'wallSliding', 'jetpack'].includes(s.state)) {
    ...
    // --- SLAM ---
    if (input.downDown && !t.onGround && a.available.has('slam') && canAct && s.state !== 'jetpack') {
    ...
    // --- WALL SLIDE ENTRY ---
    if (!t.onGround && t.onWall !== 0 && t.vel.y > 0 && a.available.has('wallSlide') && s.state !== 'climbing') {
