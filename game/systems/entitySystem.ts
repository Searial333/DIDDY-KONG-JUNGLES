...
export function entitySystem(w: World) {
    ...
    w.level.enemies.forEach(e => {
        if (activeEnemies.has(e.id) && ![...w.entities].some(eid => (get<StateMachine>(w, 'state', eid))?.enemyId === e.id)) {
            ...
            const size = isKremling ? { x: 50, y: 70 } : { x: 50, y: 50 }; // Slightly tighter hitboxes
            
            set<Transform>(w, 'transform', enemyId, { pos: { x: e.x, y: e.y }, vel: { x: 0, y: 0 }, size, facing: -1, onGround: false, onWall: 0, groundY: -1, onLadder: false, lastCheckpoint: {x:0, y:0} });
